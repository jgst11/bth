<?php
/**
 * Main class for Anton, holds everything.
 *
 * @package AntonCore
 */
class CAnton implements ISingleton {
	...
	protected function __construct() {
		...
		// include the site specific config.php and create a ref to $jst to be used by config.php
		$jst = &$this;
    require(ANTON_SITE_PATH.'/config.php');
  ...
  /**
	 ...
	 * @return CAnton The instance of this class.
	 */
	public static function Instance() {
		if(self::$instance == null) {
			self::$instance = new CAnton();
		}
  ...
  public function ThemeEngineRender() {
    ...
    $themePath 	= ANTON_INSTALL_PATH . '/' . $this->config['theme']['path'];
    ...
    if(isset($this->config['theme']['parent'])) {
      $parentPath = ANTON_INSTALL_PATH . '/' . $this->config['theme']['parent'];
      $parentUrl	= $this->request->base_url . $this->config['theme']['parent'];
    }
    
    // Add stylesheet name to the $jst->data array
    $this->data['stylesheet'] = $this->config['theme']['stylesheet'];
    
    // Make the theme urls available as part of $jst
    ...
    $jst = &$this;
    // First the default Anton themes/functions.php
    include(ANTON_INSTALL_PATH . '/themes/functions.php');
    ...
    // Extract $jst->data to own variables and handover to the template file
  ...
  /**
   * Draw HTML for a menu defined in $jst->config['menus'].
   *
