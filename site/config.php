 ...
 */
$jst->config['debug']['anton'] = false;
$jst->config['debug']['session'] = false;
$jst->config['debug']['timer'] = true;
$jst->config['debug']['db-num-queries'] = true;
$jst->config['debug']['db-queries'] = true;


/**
 * Set database(s).
 */
$jst->config['database'][0]['dsn'] = 'sqlite:' . ANTON_SITE_PATH . '/data/.ht.sqlite';
 ...
 */
$jst->config['url_type'] = 1;


/**
 * Set a base_url to use another than the default calculated
 */
$jst->config['base_url'] = null;


/**
 * How to hash password of new users, choose from: plain, md5salt, md5, sha1salt, sha1.
 */
$jst->config['hashing_algorithm'] = 'sha1salt';


/**
 * Allow or disallow creation of new user accounts.
 */
$jst->config['create_new_users'] = true;


/**
 * Define session name
 */
$jst->config['session_name'] = preg_replace('/[:\.\/-_]/', '', __DIR__);
$jst->config['session_key']  = 'anton';


/**
 * Define default server timezone when displaying date and times to the user. All internals are still UTC.
 */
$jst->config['timezone'] = 'Europe/Stockholm';


/**
 * Define internal character encoding
 */
$jst->config['character_encoding'] = 'UTF-8';


/**
 * Define language
 */
$jst->config['language'] = 'en';
 ...
 * Define menus.
 *
 * Create hardcoded menus and map them to a theme region through $jst->config['theme'].
 */
$jst->config['menus'] = array(
  'navbar' => array(
 ...
 * Template files can reside in the parent or current theme, the CAnton::ThemeEngineRender()
 ...
 *
 * path: Path to current theme, relativly ANTON_INSTALL_PATH, for example themes/grid or site/themes/mytheme.
